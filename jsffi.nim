...
# ── raw-handle seam for higher layers (web-state, DOM bindings) ──────────────
# A layer built on jsffi (e.g. aoughwl/web-state) needs to marshal a `JsValue`
# across its own `importc` boundary to the runtime. These expose the underlying
# handle with an explicit ownership contract, so such a layer never has to
# duplicate jsffi's ARC hooks.
proc unsafeHandle*(v: JsValue): int32 {.inline.} = v.h
  ## The raw table handle, **borrowed** — do NOT release it; `v` still owns it.
  ## Use to pass a JsValue's value into a runtime function that only reads it.
proc unsafeFromHandle*(h: int32): JsValue {.inline.} = JsValue(h: h)
  ## Wrap a handle as an **owning** JsValue — take this only for a handle the
  ## runtime freshly interned for you (its `=destroy` will release the slot).
